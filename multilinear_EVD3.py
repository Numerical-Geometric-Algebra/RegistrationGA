    ...
    for i in range(len(basis)):
        for j in range(len(basis)):
            for k in range(len(basis)):
                out += rbasis[k]*rbasis[j]*rbasis[i]*F(basis[i]*basis[j]*basis[k]*X)
    return out
...
def F3(X):
    ...
    for i in range(len(basis)):
        for j in range(len(basis)):
            out += rbasis[j]*rbasis[i]*F(basis[i]*basis[j]*X*r4)*pyga.inv(r4)
    return out
...
def F2(X):
    out = 0
    for i in range(len(basis)):
        out += rbasis[i]*F(basis[i]*X*r3*r4)*pyga.inv(r4)*pyga.inv(r3)
    return out
